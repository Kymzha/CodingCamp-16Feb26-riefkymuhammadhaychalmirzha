...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Edit Administrasi</h1>
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form Edit Administrasi</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('administrasis.update', $administrasi) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <select name="pasien_id"
                        ...
                        @foreach($pasiens as $pasien)
                            <option value="{{ $pasien->id }}"
                                {{ old('pasien_id', $administrasi->pasien_id) == $pasien->id ? 'selected' : '' }}>
                                {{ $pasien->nama }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="dokter_id"
                        ...
                        @foreach($dokters as $dokter)
                            <option value="{{ $dokter->id }}"
                                {{ old('dokter_id', $administrasi->dokter_id) == $dokter->id ? 'selected' : '' }}>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <input type="date" name="tanggal_periksa"
                           ...
                           value="{{ old('tanggal_periksa', \Carbon\Carbon::parse($administrasi->tanggal_periksa)->format('Y-m-d')) }}">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <textarea name="keluhan" rows="3"
                              class="form-control @error('keluhan') is-invalid @enderror">{{ old('keluhan', $administrasi->keluhan) }}</textarea>
                    @error('keluhan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <textarea name="diagnosa" rows="3"
                              class="form-control @error('diagnosa') is-invalid @enderror">{{ old('diagnosa', $administrasi->diagnosa) }}</textarea>
                    @error('diagnosa')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-sm-9">
                    <div class="input-group">
                        ...
                        <input type="number" name="biaya"
                               ...
                               value="{{ old('biaya', $administrasi->biaya) }}" min="0">
                        @error('biaya')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select name="status"
                        ...
                        <option value="Menunggu" {{ old('status', $administrasi->status) == 'Menunggu' ? 'selected' : '' }}>Menunggu</option>
                        <option value="Selesai"  {{ old('status', $administrasi->status) == 'Selesai'  ? 'selected' : '' }}>Selesai</option>
                        <option value="Batal"    {{ old('status', $administrasi->status) == 'Batal'    ? 'selected' : '' }}>Batal</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
